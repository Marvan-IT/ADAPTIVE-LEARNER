...
def build_concept_blocks_from_mmd(
    ...
    Images are Mathpix CDN URLs embedded inline in book.mmd. They are stripped
    from concept block text here — chunk_builder.py downloads them and stores
    ChunkImage rows in PostgreSQL at their exact PDF positions.
    ...
    for sec in sections:
        # Strip CDN image refs — images live in ChunkImage table, not concept text
        content = re.sub(r'!\[.*?\]\([^)]+\)', '', sec.content_mmd)
